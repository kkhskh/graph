...
class ExperimentRunner:
    ...
    def __init__(self):
        ...
        self.graph_analyzer = self._initialize_service_graph()
        print(f"[DEBUG] ServiceGraph nodes after initialization: {list(self.graph_analyzer.graph.nodes())}")
        self.statistical_analyzer = StatisticalAnomalyDetector()
        self.recovery_system = EnhancedRecoverySystem(self.graph_analyzer, self.docker_client)
    ...
    def run_experiment(self, experiment_id: int, fault_type: str = 'cpu'):
        ...
        while time.time() - propagation_start < 60:  # Monitor for 60 seconds
            ...
            stat_predictions_raw = self.statistical_analyzer.detect_anomalies(service_states)

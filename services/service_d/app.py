...
dependency_health.labels(dependency='service_c').set(1.0)  # Initialize with healthy state
...
def check_dependency_health():
    """Check health of Service C"""
    try:
        response = requests.get('http://service_c:5000/health', timeout=0.5)
        if response.status_code == 200:
            data = response.json()
            health_score = data.get('health_score', 0.0)
            dependency_health.labels(dependency='service_c').set(health_score)
            return health_score
    except:
        dependency_health.labels(dependency='service_c').set(0.0)
    return 0.0
...
def metrics():
    ...
    try:
        ...
        dependency_score = check_dependency_health()
        dependency_health.labels(dependency='service_c').set(dependency_score)

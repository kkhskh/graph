...
if _mod_name in _sys.modules:
    ...
    # Resolve the *real* legacy evaluator stub located under the top-level
    # ``graph-heal/scripts/`` directory.  We walk up ``__file__`` parents until
    # we find that directory so the logic works regardless of whether this file
    # lives in ``graph_heal/scripts`` (root package) or the historical
    # ``graph-heal/graph_heal/scripts`` tree used by older CI layouts.

...
def enrich_service(svc):
    ...
    instance_map = {
        'user_service': 'service_a:5000',
        'order_service': 'service_b:5000',
        'inventory_service': 'service_c:5000',
        'notification_service': 'service_d:5000'
    }
    ...
    cpu_usage = prom_scalar(f'service_cpu_usage{{instance="{instance}"}}', svc["timestamp"])
    rtt = prom_scalar(f'service_response_time{{instance="{instance}"}}', svc["timestamp"])
    mem_usage = prom_scalar(f'service_memory_usage{{instance="{instance}"}}', svc["timestamp"])
...
def enrich_snapshot(snapshot):
    """
    Deal with ONE snapshot.
    If snapshot is {service: {...}} return same shape.
    If snapshot is {service: [{...}, {...}]} return same shape.
    """
    enriched = {}
    for svc_name, svc_data in snapshot.items():
        if isinstance(svc_data, list):
            ...
            enriched[svc_name] = [enrich_service(m) for m in svc_data]
        elif isinstance(svc_data, dict):
            # Handle single metric
            enriched[svc_name] = enrich_service(svc_data)
    return enriched
